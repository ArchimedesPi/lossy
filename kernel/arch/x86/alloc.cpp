...
extern "C" {
	...
	void *ksbrk(int n) {
		...
		if ((kern_heap + (n * PAGESIZE)) > (char *) KERN_HEAP_LIM) {
			logger.panic("ksbrk(): no virtual memory left for kernel heap !");
			return (char *) -1;
		}
		...
		for (i = 0; i < n; i++) {
			...
			if ((int)(p_addr) < 0) {
				logger.panic("ksbrk(): no free page frame available !");
				return (char *) -1;
			}
	...
	void *kmalloc(unsigned long size) {
		...
		if (chunk->size == 0) {
		logger.panic("kmalloc(): corrupted chunk on %x with null size (heap %x) !\nSystem halted\n", chunk, kern_heap);
		return 0;
		}
