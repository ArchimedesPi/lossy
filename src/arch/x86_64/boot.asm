...
; set up an extremely small (64 bit) stack
section .bss:
align 4096
p4_table:
    resb 4096
...
set_up_page_tables:
    ; map last P4 entry to the table itself
    mov eax, p4_table
...
; prints `ERR: ` and an error code to screen and then hangs.
; parameters:
;  char in al: error code
error:
...
section .rodata
gdt64: ; a 64-bit gdt so the cpu is happy in long mode
    dq 0 ; zero entry
.code: equ $ - gdt64 ; new
